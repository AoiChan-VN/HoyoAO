...
export const tests = [
  { name: 'registerApp grants known permissions only', fn() {
  ...
  { name: 'cannot grant unknown permission', fn() {
    const svc = new PermissionService(createMockEventBus(), createMockLogger());
    svc.registerApp('app5', []);
    assert(!svc.grant('app5', 'totally.unknown'));
  }},
  { name: 'audit log records events', fn() {
    const svc = new PermissionService(createMockEventBus(), createMockLogger());
    svc.registerApp('app6', ['data.read']);
    svc.has('app6', 'network');
