...
export const tests = [
  {
    name: 'register and resolve route',
    fn() {
      ...
      assert(route !== null, 'Should resolve');
      assertEqual(route.scope, 'os');
    },
  },
  {
    name: 'rejects invalid route',
    fn() {
      const reg = new RouteRegistry(createMockLogger());
      reg.register(null);
      reg.register({ path: '/x' }); // missing scope and kind
      assertEqual(reg.getAll().length, 0);
    },
  },
  {
    name: 'getOSRoutes filters correctly',
    fn() {
      const reg = new RouteRegistry(createMockLogger());
      ...
      assertEqual(os[0].path, '/os/b'); // sorted by order
    },
  },
  {
    name: 'getApplicationRoutes filters correctly',
    fn() {
      const reg = new RouteRegistry(createMockLogger());
