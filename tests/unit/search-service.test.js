...
export const tests = [
  {
    name: 'indexItem and query by title',
    fn() {
  ...
  {
    name: 'removeItem works',
    fn() {
      const svc = new SearchService(createMockLogger(), createMockEventBus());
      svc.registerSource('test');
      svc.indexItem({ id: '6', source: 'test', type: 'x', title: 'Remove me' });
      svc.removeItem('6');
      assertEqual(svc.query('remove').length, 0);
    },
  },
  {
    name: 'scoring: title > tags > body',
    fn() {
      const svc = new SearchService(createMockLogger(), createMockEventBus());
      svc.registerSource('test');
      ...
      assertEqual(results[0].id, 'a'); // title = 10
      assertEqual(results[1].id, 'c'); // tag = 5
      assertEqual(results[2].id, 'b'); // body = 1
    },
  },
];
